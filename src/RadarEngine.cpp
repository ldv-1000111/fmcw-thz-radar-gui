...
RadarEngine::RadarEngine(QObject* parent)
    : QObject(parent)
{
    // Defaults matching engine repo main.cpp exactly
    m_params.f0          = 300.0e9;   // 300 GHz
    m_params.bandwidth   = 4.0e9;     //   4 GHz
    m_params.chirp_time  = 100.0e-6;  // 100 µs
    m_params.fs          = 50.0e6;    //  50 MHz
    m_params.num_samples = 5000;      //  fs * chirp_time
    m_params.num_chirps  = 256;

    m_target.range       = 50.0;      // m
    m_target.velocity    = 0.0;       // m/s
    m_target.rcs         = 1.0;       // m^2 — unit reflectivity
    m_target.vib_amp     = 2.0e-4;    // 0.2 mm
    m_target.vib_freq    = 200.0;     // Hz
}
...
// ---------------------------------------------------------------------------
// Compute — chirp index 0, num_samples from RadarParams
// ---------------------------------------------------------------------------
void RadarEngine::compute()
{
    try {
        m_ifBuf.resize(m_params.num_samples);
        generate_chirp_if(m_params, m_target, 0, m_ifBuf);
        buildSpectrum();
        emit signalReady();
...
QVector<double> RadarEngine::ifSignalReal() const
{
    ...
    for (const auto& s : m_ifBuf) v.append(static_cast<double>(s.real()));
    return v;
}
...
QVector<double> RadarEngine::ifSignalImag() const
{
    QVector<double> v;
    v.reserve(static_cast<int>(m_ifBuf.size()));
    for (const auto& s : m_ifBuf) v.append(static_cast<double>(s.imag()));
    return v;
}
...
void RadarEngine::buildSpectrum()
{
    ...
    for (int k = 0; k < half; ++k) {
        ...
        for (int n = 0; n < N; ++n) {
            const double angle = -2.0 * M_PI * k * n / N;  // note: negative sign for forward DFT
            const double sr = static_cast<double>(m_ifBuf[n].real());
            const double si = static_cast<double>(m_ifBuf[n].imag());
